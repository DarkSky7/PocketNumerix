...
//	COptionChainDialog* p_OCR = (COptionChainDialog*)threadCtx;
	...
	fp = _wfopen( theApp.lpFileName, _T("w") );
	if ( !fp )
	{
#ifdef _DEBUG
...
#endif
		wchar_t	wbuf[120];
		swprintf( wbuf, _T("Can't open(w) %s"), theApp.lpFileName ); 
		SendMessage( c_StatusResult, WM_SETTEXT, 0, (LPARAM)wbuf );
...
Cleanup:
...
#ifdef _DEBUG
	TRACE( _T("DownloadURL: OnParseData bytesRead=%d\n"), bytesRead );
#endif
	if ( retVal == 0 )
	...
	if ( bytesRead > 0 )
	{
		if ( PostMessage( hwnd, WM_NILLAHEDGE_PARSE_DATA, 0, 0 ) != 0 )		// that's bad
		{
#ifdef _DEBUG
			TRACE( _T("DownloadURL: OnParseData returned %d\n"), nRet );
#endif
			retVal = -10;
		}
	}
