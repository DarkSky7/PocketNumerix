...
//	virtual CBOS*	GetBOS( long offset );			// deleted, use ReadBOS
	...
	CDBFile*	GetStkOptIndxFile( CString stkSym );				// XXX - working here
...
public:
	...
	long	GetPosIndexedDef_ID( long zbi );									// returns the def_ID located at 4*zbi in the posIndxFile
	long	GetStkIndexedOptDef_ID( CDBFile* stkOptIndxFile, long zbi );		// returns the def_ID located at 4*zbi in the stock's option index file
