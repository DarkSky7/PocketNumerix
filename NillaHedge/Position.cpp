...
{		// update the database
	if ( inst == Option )
		return	(CDataManager*)theApp.optMgr;
	else if ( inst == Stock )
		return	(CDataManager*)theApp.stkMgr;
	else
		return	NULL;
}			// getDataManager()
//---------------------------------------------------------------------------
...
CBOS*		CPosition::getBOS( void )
{
	...
	CBOS*	theAsset = NULL;
	if ( inst == Option )
		theAsset = (CBOS*)((COptionManager*)dm)->ReadOption( def_ID );
	else if ( inst == Stock )
		theAsset = (CBOS*)((CStockManager*)dm)->ReadStock( def_ID );
	else ASSERT( false );
	return	theAsset;
}			// getBOS()
//---------------------------------------------------------------------------
