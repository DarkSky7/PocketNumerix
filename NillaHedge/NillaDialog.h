...
protected:
	...
	virtual float		getComboBoxFloat( int dialogItemID );
	...
	virtual void		setStatic( bool valid, char* format, double dd, int dialogItemID );
	virtual void		setStatic( CString cs, int dialogItemID );
	virtual void		RegisterHotKeys( DWORD ctrlId1, DWORD ctrlId2 );
...
//	virtual void		seekComboBoxEntry( int dialogItemID );								// use CSeekComboBox::SeekEntry()
	HBRUSH	m_hbGray;		// gray
	HBRUSH	m_hbWhite;		// white
};
