...
{		// ensure that the next sort on the first column will be ascending
	...
	optns[op++] = (areVisible & ExerciseValueOverview)		!= 0;		//	  8
	optns[op++] = (areVisible & NoteOverview)				!= 0;		//	  9
...
{		// in the order they will be displayed in the list control
	const wchar_t*		colHdrs[] = {
		_T("Date/Name"), _T("Units "),	_T("Cost "),   _T("MktValue"), _T("CapGain"),
		_T("Income"),	 _T("NetGain"), _T("% Yield"), _T("ExerVal"),  _T("Note/Desc")	};
	...
	if (  areVisible & ExerciseValueOverview )
		c_PositionsList.InsertColumn( nCols++, colHdrs[hdrIdx], LVCFMT_RIGHT, 66 );
...
/*
	...
	{	bondSummary = new PortfolioListItemInfo;
		...
		bondSummary->dateName = bondSummary->noteDesc = _T("Bonds");
...
*/
		// load options
	PortfolioListItemInfo*	optionSummary = NULL;
	int	res = theApp.optMgr->NumIndexedIssues();
	if ( res > 0 )
	{	optionSummary = new PortfolioListItemInfo;
		optionSummary->inst = Option;
		optionSummary->level = Instruments;			// i.e. all Options
		optionSummary->dateName = optionSummary->noteDesc = _T("Options");
		optionSummary->nUnits	  = 0.0;
		optionSummary->totalCost  = 0.0;
		optionSummary->mktValue   = 0.0;
		optionSummary->exerIncome = 0.0;
		double			 wtdYield = 0.0;
		for ( zbi = 0; ; zbi++ )
		{	COption*	anOption = theApp.optMgr->GetPosIndexedOption( zbi ); 
			if ( anOption == NULL )
				break;
			PortfolioListItemInfo*	issueSummary = LoadPositions( anOption );
			optionSummary->nUnits	  += issueSummary->nUnits;
			optionSummary->totalCost  += issueSummary->totalCost;
			optionSummary->mktValue	  += issueSummary->mktValue;
			optionSummary->exerIncome += issueSummary->exerIncome;
			wtdYield += issueSummary->totalCost * issueSummary->pctYield;
			delete	anOption;		// deletes the position list too
		}
		tCost = optionSummary->totalCost;
		optionSummary->pctYield = (float) ((fabs(tCost) < 1e-5)  ?  0.0  :  (wtdYield / tCost));
			// put the option option summary row into the rowData array and the PositionsList
		c_PositionsList.AddItem( optionSummary );
	}

		// load stocks
	PortfolioListItemInfo*	stockSummary = NULL;
	res = theApp.stkMgr->NumIndexedIssues();
	if ( res > 0 )
	{	stockSummary = new PortfolioListItemInfo;
		stockSummary->inst = Stock;
		stockSummary->level = Instruments;		// i.e. all Stocks
		stockSummary->dateName = stockSummary->noteDesc = _T("Stocks");
		stockSummary->nUnits	 = 0.0;
		stockSummary->totalCost  = 0.0;
		stockSummary->mktValue   = 0.0;
		stockSummary->exerIncome = 0.0;
		double			wtdYield = 0.0;
		for ( zbi = 0; ; zbi++ )
		{	CStock*	aStock = theApp.stkMgr->GetPosIndexedStock( zbi ); 
			if ( aStock == NULL )
				break;
			PortfolioListItemInfo*	issueSummary = LoadPositions( aStock );
			stockSummary->nUnits	 += issueSummary->nUnits;
			stockSummary->totalCost  += issueSummary->totalCost;
			stockSummary->mktValue	 += issueSummary->mktValue;
			stockSummary->exerIncome += issueSummary->exerIncome;
			wtdYield += issueSummary->totalCost * issueSummary->pctYield;
			delete	aStock;		// deletes the position list too
		}
		tCost = stockSummary->totalCost;
		stockSummary->pctYield = (float) ((fabs(tCost) < 1e-5)  ?  0.0  :  (wtdYield / tCost));
			// put the stock summary row into the rowData array and the PositionsList
		c_PositionsList.AddItem( stockSummary );
	}

		// finally, insert a portfolio summary
	portfolio->inst = UnknownAssetType;
	portfolio->level = Portfolio;
	portfolio->dateName = portfolio->noteDesc = _T("Portfolio");
	portfolio->nUnits	  = (optionSummary ? optionSummary->nUnits	  : 0.0)
						  + (stockSummary  ? stockSummary->nUnits	  : 0.0);

	portfolio->totalCost  = (optionSummary ? optionSummary->totalCost : 0.0)
						  + (stockSummary  ? stockSummary->totalCost  : 0.0);
	portfolio->mktValue   = (optionSummary ? optionSummary->mktValue  : 0.0)
						  + (stockSummary  ? stockSummary->mktValue	  : 0.0);
	portfolio->exerIncome = (stockSummary  ? stockSummary->exerIncome : 0.0);	// exerVal isn't income

	double	wtdYield = (optionSummary ? optionSummary->totalCost : 0.0)
					 * (optionSummary ? optionSummary->pctYield	 : 0.0)
					 + (stockSummary  ? stockSummary->totalCost	 : 0.0)
					 * (stockSummary  ? stockSummary->pctYield	 : 0.0);
	tCost = portfolio->totalCost;
	portfolio->pctYield = (float) ( (fabs(tCost) < 1e-3)  ?  0.0  : (wtdYield / tCost) );
		// put the portfolio summary row into the rowData array and the PositionsList
	c_PositionsList.AddItem( portfolio );
}			// LoadRows()
//----------------------------------------------------------------------------
...
{	AssetType	inst = bos->getAssetType();
	...
	issueSummary->inst = inst;					// Option/Stock
	issueSummary->level = Issues;				// issueSummary for an issue (an Option or Stock)
	...
	{		// 'copy' the easy stuff
		...
		double	exerVal = 0.0;
//		if ( inst == Bond )
//			income = ((CBond*)bos)->PresentValueCoupons( pos->purchaseDate, today, 0.0 );
//		else
		if ( inst == Option )
		{	if ( ((COption*)bos)->ExerVal(exerVal) )
				exerVal *= pos->nUnits;
		}
		else	// Stock
			income = pos->nUnits * ((CStock*)bos)->PresentValueDivs( pos->purchaseDate, today, 0.0 );
		plii->exerIncome = (inst == Option ) ? exerVal : income;
...
{		// return a CString representation of the data in row rr, subitem si
	...
	if ( msi < 0  ||  msi > 9 )				// not including the dummy column
		return	_T("");
	...
	if ( msi == 0 )
		...
		// 9th (zero based) column - note for positions, desc for issues
	if ( msi == 9 )
		return	pItem->noteDesc;

		// reject aggIncome for Options & exerVal for other instruments
	if ( pItem->inst == Option )
	{	if ( msi == 5 )					// Options don't show aggIncome entries
			return	_T("");
	}
	else
	{	if ( msi == 8 )					// only Options have an exerVal entry
			return	_T("");
	}
	...
	{		// number of options/shares
		...
		{		// have already screened out Options above
			...
			// netGain
		double	netGain = capGain + (pItem->inst == Option ? 0.0 : pItem->exerIncome);
		if ( msi == 6 )
		...
		{	double	plusYld = fabs( pItem->pctYield );
			...
			// exerValue
		if ( msi == 8 )
		{		// have already screened out non-Options above
			swprintf( buf, _T("%.2f"), pItem->exerIncome );
			break;
		}
