// Matrix_f.cpp: implementation of the CMatrix_f class.
//
//////////////////////////////////////////////////////////////////////
...
// Construction/Destruction
	...
	unsigned short	nCols			 ) 
		: rows(nRows), cols(nCols), elems( new float*[rows] )
{	elems[0] = new float[rows * cols];
		// set up dereferencing index
	for ( unsigned short rr = 1; rr < rows; rr++ )
	...
	// transform (unary apply) constructor
			CMatrix_f::CMatrix_f(
	CMatrix_f&	am,
	float				func( float elem )		)
		: rows(am.rows), cols(am.cols), elems( new float*[rows] )
{	elems[0] = new float[rows * cols];
		// set up dereferencing index
	for ( unsigned short rr = 1; rr < rows; rr++ )
	{	elems[rr] = elems[rr-1] + cols;
	...
	// deep copy constructor
			CMatrix_f::CMatrix_f(
	CMatrix_f&	exmp			 )
		: rows(exmp.rows), cols(exmp.cols), elems( new float*[rows] )
{	elems[0] = new float[rows * cols];
		// set up dereferencing index
	for ( unsigned short rr = 1; rr < rows; rr++ )
	{	elems[rr] = elems[rr-1] + cols;
...
//--------------------------------------------------------------------
CMatrix_f*		CMatrix_f::operator*( CMatrix_f& right )
{	ASSERT( elems );
	ASSERT( right.elems );
	if ( cols != right.rows )
		return 0;
	CMatrix_f*	prod = new CMatrix_f( rows, right.cols );
	float**		prdEl = prod->elems;
	float**		rgtEl = right.elems;
...
//--------------------------------------------------------------------
CVector_f*		CMatrix_f::operator*( CVector_f& right )
{	ASSERT( elems );
	ASSERT( right.elems );
	if ( cols != right.nElems )
		return 0;
	CVector_f*	prod = new CVector_f( cols );
	float*		prdEl = prod->elems;
	float*		rgtEl = right.elems;
	for ( unsigned short rr = 0; rr < rows; rr++ )
...
//--------------------------------------------------------------------
void		CMatrix_f::apply( float func( float elem ) )
{	for ( unsigned short rr = 0; rr < rows; rr++ )
	{	for ( unsigned short cc = 0; cc < cols; cc++ )
...
//--------------------------------------------------------------------
CMatrix_f*		CMatrix_f::transpose( void )
{	ASSERT( elems );
	CMatrix_f* trans = new CMatrix_f( cols, rows );
	float**  trnEl = trans->elems;
	for ( unsigned short rr = 0; rr < rows; rr++ )
		for ( unsigned short cc = 0; cc < cols; cc++ )
...
{	ASSERT( elems );
	...
	for ( unsigned short cc = 0; cc < cols; cc++ )
	{	float	  tmp = elems[r1][cc];
		elems[r1][cc] = elems[r2][cc];
		elems[r2][cc] = tmp;
	}
}			// swapRows()
//--------------------------------------------------------------------
void			CMatrix_f::scaleRow(
	unsigned short	row,
	float			factor			)
...
//--------------------------------------------------------------------
CMatrix_f*		CMatrix_f::inverse( void )
{	ASSERT( elems );
	if ( rows != cols )
		return 0;
	CMatrix_f*	copy = new CMatrix_f( *this );		// create a deep copy
	CMatrix_f*	invs = new CMatrix_f( rows, cols );
	float**		invEl = invs->elems;
	...
	{		// pick the largest element in this column to pivot on
		float	pivot = copy->colMax( cc, cc, bigRow );
		if ( pivot == 0.0f )
...
//---------------------------------------------------------------------
/*
inline float			CMatrix_f::COMPL_A( float aa, float bb )
{	return bb >= 0.0 ? (aa >= 0.0 ?  aa : -aa)
					 : (aa >= 0.0 ? -aa :  aa);
}			// COMPL_A()
//--------------------------------------------------------------------
float			CMatrix_f::pythag( float aa, float bb )
{		// compute sqrt( a^2 + b^2 ) without underflow or overflow
	float	absA = (float)fabs((double)aa);
	float	absB = (float)fabs((double)bb);
	if ( absA > absB )
	{	if ( absA == 0.0 )
			return 0.0;
		float	rr = absB / absA;
		return absA * (float)sqrt( 1.0 + (double)rr*(double)rr );
	}
	else
	{	if ( absB == 0.0 )
			return 0.0;
		float	rr = absA / absB;
		return absB * (float)sqrt( 1.0 + (double)rr*(double)rr );
	}
}			// pythag()
//--------------------------------------------------------------------
void			CMatrix_f::svbksb(
//	CMatrix_f&	uu,
	CMatrix_f&	vv,
	CVector_f&	ww,
	CVector_f&	bb,
	CVector_f&	aa				)				// the return vector
{	CMatrix_f&	uu = *this;
	unsigned short	ii, jj, pp;					// NR::j --> jj,	NR::jj -> pp
	unsigned short	uuRows = uu.rows;			// NR::m
	unsigned short	uuCols = uu.cols;			// NR::n
	CVector_f		tmp( uuCols );
	float				ss;
...
void			CMatrix_f::svdcmp(
//	CMatrix_f&	aa,						// A upon input, U upon return
	CMatrix_f&	vv,
	CVector_f&	ww				)
{	CMatrix_f&	aa = *this;				// A upon input, U upon return
	unsigned short	ii, jj, kk, ll, pp;
	unsigned short	aaRows = aa.rows, aaCols = aa.cols;
	float			aNorm = 0.0, ff, gg, hh, ss, scale;
	CVector_f		rv1( aaCols );
		//
	...
	{	ll = ii + 2;
		...
		if ( ii < aaRows )
		{	for ( kk = ii; kk < aaRows; kk++ )	scale += (float)fabs( (double)aa.elems[kk][ii] );
			if ( scale != 0.0 )
			{	for ( kk = ii; kk < aaRows; kk++ )
				...
				ff = aa[ii][ii];
				gg = - COMPL_A( (float)sqrt((double)ss), ff );					// gg = -sqrt(ss) or sqrt(ss)
				hh = ff * gg - ss;
				aa[ii][ii] = ff - gg;
		...
		{	for ( kk = ll - 1; kk < aaCols; kk++ )	scale += (float)fabs( (double)aa.elems[ii][kk] );	// ( kk = ii; kk < aaRows; kk++ ) ... aa[kk][ii]
			if ( scale != 0.0 )
			{	for ( kk = ll - 1; kk < aaCols; kk++ )								// ( kk = ii; kk < aaRows; kk++ )
				...
				gg = - COMPL_A( (float)sqrt((double)ss), ff );
				hh = ff * gg - ss;
		...
		aNorm = (float)MAX( aNorm, (float)(fabs((double)ww.elems[ii]) + fabs((double)rv1.elems[ii])) );
	}
		//
	...
	{	unsigned short nm;
		...
		{	bool	flag = TRUE;
			...
			{	nm = ll - 1;					// note that rv1[0] is always zero
				if ( (float)fabs((double)rv1.elems[ll]) + aNorm == aNorm )
				{	flag = FALSE;
					break;
				}
				if ( (float)fabs((double)ww.elems[nm]) + aNorm == aNorm )
					break;
			}
			float yy, zz;
			if ( flag )
			{		// Cancellation of rv1.elems[ll], if ll > 0.
				...
				{	ff = ss * rv1[ii];
					rv1.elems[ii] *= cc;
					if ( (float)fabs((double)ff) + aNorm == aNorm )
						break;
					gg = ww.elems[ii];
					hh = pythag( ff, gg );
					ww.elems[ii] = hh;
					hh = float(1.0 / (double)hh);
					cc =   gg * hh;
					ss = - ff * hh;
			...
			float xx = ww.elems[ll];		nm = kk - 1;		yy = ww.elems[nm];
			gg = rv1.elems[nm];		hh = rv1.elems[kk];
			ff = (float)(((yy - zz) * (yy + zz) + (gg - hh) * (gg + hh)) / (2.0 * hh * yy));
			gg = pythag( ff, 1.0 );
			ff = ( (xx - zz) * (xx + zz) + hh * ((yy / (ff + COMPL_A(gg,ff))) - hh) ) / xx;
			float	cc = ss = 1.0f;						// Next QR transformation
			for ( jj = ll; jj <= nm; jj++ )
			{	ii = jj + 1;
				gg = rv1.elems[ii];
				yy = ww.elems[ii];
				...
				zz = pythag( ff, hh );
				rv1.elems[jj] = zz;
				cc = (float)((double)ff / (double)zz);
				ss = (float)((double)hh / (double)zz);
				ff = xx * cc + gg * ss;
				gg = gg * cc - xx * ss;
				hh = yy * ss;
				yy *= cc;
...
//--------------------------------------------------------------------
float				CMatrix_f::svdfit(				// the return value is chiSq
	CVector_f&  xx,	CVector_f&  yy,	CVector_f&  sig,
	// CMatrix_f&  uu,
...
{	CMatrix_f&  uu = *this;
	...
	CVector_f		bb(nDataPts), xxVec(nDataPts);

		// create the A matrix (in uu) for the svdcmp routine.
	for ( ii = 0; ii < nDataPts; ii++ )
	{	func( xx[ii], xxVec );
		float	ooSig = (float)(1.0 / (double)sig.elems[ii]);
			// fitVec used here
	...
	{		// func( xx[ii], xxVec );			// why do this twice?  reuse the first one
		...
		float	zz = (yy[ii] - sum) / sig.elems[ii];
		chiSq += zz*zz;
	}
	return chiSq;
}			// svdfit()
//--------------------------------------------------------------------
CMatrix_f*	CMatrix_f::svdvar(
	CMatrix_f&	vv,
	CVector_f&	ww				)
{	unsigned short ii, jj, kk, wItems = ww.nElems;
		// cvm (the covariance matrix) is the return value
	CMatrix_f*	cvm = new CMatrix_f( wItems, wItems );	// heap based return matrix

	CVector_f wti( wItems );								// stack based
