...
{	short ii;
	const short numParams = 6;
		// ?os=%s?dv=%d?uu=%s?hu=%s?stk=%s%s or some reordering thereof
		// expBuf contains its own ?exp
	char osBuf[ 16 ];
	int slen = sprintf( osBuf, "?os=%s", osVer );

	char uuBuf[ 40 ];
	slen = sprintf( uuBuf, "?uu=%s", uuid );

	char huBuf[ 48 ];
	slen = sprintf( huBuf, "?hu=%s", ( huid == NULL ? "0" : huid ) );

	char dvBuf[ 24 ];
	slen = sprintf( dvBuf, "?dv=%d", dv );

	char stkBuf[ 264 ];
	slen = sprintf( stkBuf, "?stk=%s", stk );

		// expBuf either looks like "?exp=%s" or is NULL
	char* in[numParams] = { osBuf, dvBuf, uuBuf, huBuf, stkBuf, expBuf };
	char* out[numParams] = { NULL };
	short len[numParams] = { 0 };
	slen = 0;
	for ( ii = 0; ii < numParams; ii++ )
	{	char* src = *(in+ii);
		while ( 1 )
		{	int rr = rand() % numParams;
			if ( *(out+rr) == NULL )
			{	*(out+rr) = src;
				*(len+rr) = strlen( src );
				slen += *(len+rr);
				break;						// just the outer loop
			}
		}
	}
