...
// CFB MODE 
int _init_mcrypt( CFB_BUFFER* buf, byte* key, int lenofkey, void *IV, int size )	// void* key to char* key
{		// e.g. _init_mcrypt( buf, ??, f(??), IV, 256 ); 
    buf->s_register = buf->enc_s_register = NULL;   
    buf->blocksize = size;

		// For cfb
	buf->s_register = (byte*)malloc( size );
    if ( buf->s_register == NULL )
		goto	freeall;

	buf->enc_s_register = (byte*)malloc( size );
    if ( buf->enc_s_register == NULL )
		goto	freeall;
...
//--------------------------------------------------------------------------------------------------------------------------
int _mcrypt( CFB_BUFFER* buf, char* plaintext, int len, int blocksize, void* akey, void (*func)(void*,void*), void (*func2)(void*,void*) )
...
//--------------------------------------------------------------------------------------------------------------------------
int _mdecrypt( CFB_BUFFER* buf, char *plaintext, int len, int blocksize, void* akey, void (*func)(void*,void*), void (*func2)(void*,void*))
...
//--------------------------------------------------------------------------------------------------------------------------
int _is_block_algorithm_mode()
{
	return	1;
}
//--------------------------------------------------------------------------------------------------------------------------
const char *_mcrypt_get_modes_name()
{
	return	"CFB";
}
//--------------------------------------------------------------------------------------------------------------------------
int _mcrypt_mode_get_size()
{
	return	sizeof( CFB_BUFFER );
}
//--------------------------------------------------------------------------------------------------------------------------
