...
{		// input: cipherKey 
		// output: keySchedule
	for ( short ii = 0; ii < nRjKeys; ii++ )				// nKeys = 8
	{		// copy cipherKeys into the first words of the key schedule
		*(keySchedule+ii) = *((unsigned int*)cipherKey+ii);
	}
	...
	for ( short ii = nRjKeys; ii < maxRoundKey; ii++ )
	{	unsigned int temp = *(keySchedule+ii-1);
		if ( (ii % nRjKeys) == 0 )
		{	unsigned int rconVal = *(Rcon + ii/nRjKeys - 1);
			unsigned int substWord = SubWord( RotateWord(temp) );
			temp = substWord ^ rconVal;		// the first usable ii == nKeys
		}
		else if ( nRjKeys > 6  &&  (ii % nRjKeys) == 4 )
			temp = SubWord( temp );
		*(keySchedule+ii) = *(keySchedule+ii-nRjKeys) ^ temp;
	}
}			// rjKeyExpansion()
//------------------------------------------------------------------------------------------------
...
void	inline XorRoundKey( unsigned int* state, const unsigned int* keySchedule )
{	for ( unsigned short cc = 0; cc < nRjColumns; cc++ )
		*(state+cc) ^= *(keySchedule+cc);
}			// XorRoundKey()
//------------------------------------------------------------------------------------------------
// requires generation of a keySchedule
void	rjCipher( unsigned char* ibuf, unsigned char* obuf, const unsigned int* keySchedule )
{	unsigned char	state[ nRjCells ];
	memcpy( state, ibuf, nRjCells );
#define LOCAL_DEBUG		0
	XorRoundKey( (unsigned int*)state, keySchedule );				// round 0 
#if LOCAL_DEBUG
	...
	{	SubBytes( state );
		...
		XorRoundKey( (unsigned int*)state, keySchedule + rn * nRjColumns );
#if LOCAL_DEBUG
		fprintf( stderr, "\r\nXorRoundKey(%d) -->\r\n", rn );
		for ( short ii = 0; ii < AES_BLOCK_SIZE; ii++ )
			fprintf( stderr, " %02x", *(state+ii) );
	...
	XorRoundKey( (unsigned int*)state, keySchedule + nRjRounds * nRjColumns );
#if LOCAL_DEBUG
	fprintf( stderr, "\r\nXorRoundKey(%d) -->\r\n", nRounds );
	for ( short ii = 0; ii < AES_BLOCK_SIZE; ii++ )
		fprintf( stderr, " %02x", *(state+ii) );
	...
	while ( iBytes > 0 )
	{
		...
		{	if ( aligned_04(ibuf)  &&  aligned_04(obuf)  &&  aligned_04(iv) )
			{		// we ALWAYS take this branch
				for ( short ii = 0; ii < nRjColumns; ii++ )
				{	unsigned int tt = lp32(ibuf)[ii];
					lp32(obuf)[ii] = tt ^ lp32(iv)[ii];
					lp32(iv)[ii] = tt;
				}
			}
			else
			{	for ( short ii = 0; ii < nRjRows*nRjColumns; ii++ )
				{	unsigned char tt = ibuf[ii];
					obuf[ii] = tt ^ iv[ii];
					iv[ii] = tt;
				}
			}
...
void	rcKeyExpansion( unsigned int* cipherKey, unsigned int* roundKeys )
{		//	Input:   array L[ 0 … c-1 ] of cipher key words, c :== nRcBitsInKey / nBitsPerWord = 8;
	...
	for ( unsigned int ll = 0; ll < nRcWordsInKey; ll++ )
		*(LL+ll) = *(cipherKey+ll);								// copy the cipherKey
	...
	*(roundKeys+0) = Pw;						// seed the roundKeys with the fractional part of e
	for ( unsigned int rr = 1; rr < nRcRoundKeys; rr++ )
		*(roundKeys+rr) = *(roundKeys+rr-1) + Qw;				// add the fractional part of phi to create successive roundKeys

		// modify roundKeys using the local copy of the cipherKey
	unsigned int AA, BB, ll, rr;
	...
	{	unsigned int AApBB = AA + BB;
		AA = *(roundKeys+rr++) = RotateLeft( *(roundKeys+rr) + AApBB, 3 );
...
void	rcCipher( unsigned int* ibuf, unsigned int* obuf, const unsigned int* rk )
...
{		// Refer to: http://en.wikipedia.org/wiki/Block_cipher_modes_of_operation
	...
	while ( iBytes > 0 )
	{	rcCipher( (unsigned int*)iv, (unsigned int*)iv, roundKeys );
