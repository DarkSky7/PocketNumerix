...
{	FILE*	fp;												// closed at CloseFile
	...
	BOOL success;
		...
		// retrieve handles of windows we'll send messages to...
	hwnds_t* zz = (hwnds_t*)threadCtx;
	HWND	hwnd = zz->ycfDialog;
	HWND	c_Fetch = zz->fetchButton;
	HWND	c_StatusResult = zz->statusResult;
	int		retVal = 0;
	
	EnableWindow( c_Fetch, FALSE );
	...
	unsigned int	addr;
	addr = inet_addr( theApp.lpServerName );						// XXXX
	if ( addr != INADDR_NONE )
	...
	{		// maybe it was an FQDN like OptimalPortfolio.net
		lpHostEntry = gethostbyname( theApp.lpServerName );		// XXXX
		if ( lpHostEntry != NULL )
		{		// finish setting up saServer (for use in connect)
	...
	{		// time limit expired
...
#ifdef _DEBUG
		TRACE( _T("DownloadURL(-6): SEND Socket Timeout\n") );
#endif
		retVal = -6;
		goto	CloseSocket;
	}
		...
		// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
		// Format a GET request (which internally is really a POST)
	char szBuffer[ 256 ];
	sprintf( szBuffer, "GET %s\n", theApp.urlRequest );		// XXXX
#ifdef _DEBUG
	{	CString	cs(szBuffer);
		TRACE( _T("Sending: %s\n"), cs );
	}
	...
	fp = _wfopen( theApp.lpFileName, _T("w") );								// XXXX
	if ( !fp )
	{
#ifdef _DEBUG
...
#endif
		wchar_t	wbuf[120];
		swprintf( wbuf, _T("Can't open(w) %s"), theApp.lpFileName );		// XXXX
		SendMessage( c_StatusResult, WM_SETTEXT, 0, (LPARAM)wbuf );
		retVal = -8;
		goto	CloseSocket;
	}
	...
	{		// Wait to receive, nRet = NumberOfBytesReceived
		...
		if ( nRet == 0 )
		{		// time limit expired
			SendMessage( c_StatusResult, WM_SETTEXT, 0, (LPARAM)_T("RECV Timeout/Complete") );
			retVal = -9;
			goto	CloseFile;
		}
	...
	fp = NULL;
		...
		// parse phase
	if ( bytesRead > 0 )
	{		// lpFileName is global
		if ( PostMessage( hwnd, WM_YIELDCURVEFITTER_PARSE_DATA, 0, 0 ) != 0 )		// that's bad
		{
#ifdef _DEBUG
			TRACE( _T("DownloadURL: ParseData returned %d\n"), nRet );
#endif
			nRet = -11;
			goto	CloseSocket;
		}
