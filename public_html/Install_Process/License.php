...
$debug = false;
$skipDecrypt = false;
$useSantaFe = false;				# false --> we're living on pnmx.com on a BlueHost server
# sample request for YieldCurveFitter:
# "http://pnmx.com/Install_Process/License.php?ds=tu+stEykMyEVuPLTeO+uXiQQAsY4FaSPqeCsL99jgJZWX2eDT4cLsJMFpBwlm3pnHAhhXjV+gMsJfHddDMRHL9tiwS0aqy/ieNYw27eWwwoYSQ5wc1cNEvZWZPME"
...
{	# $queryString = $HTTP_SERVER_VARS[ 'QUERY_STRING' ];		# doesn't work
	...
	#---------------------------------------------------------------------------+
	$clrFile = $ranName . ".k";
	$lastline = system( "../cgi-bin/rc d ./cfg/LicDs.cfg $crpFile $clrFile" );
	if ( $lastLine != "" )
	{   if ( $debug )
...
{	if ( $debug )
	...
	$devicesid = $row['id'];						# so we can update the devices and license_requests records
	if ( $debug )
...
{	$result = mysql_query( $query, $link )	or die( "Devices UI failed: " . mysql_error() );		# run the INSERT or UPDATE devices query
	...
	{	# the previous query was an INSERT, so we still don't know the devicesid
		...
		if ( $numRes > 0 )
		{	$row = mysql_fetch_assoc( $result );
			$devicesid = $row['id'];						# so we can insert a license_requests record
		}
	}
}
#-------------------------------------------------------------------------------+
#						log 'request' in license_requests						|
#-------------------------------------------------------------------------------+
$query = "INSERT INTO pnmx_license_requests ( ipAddress, accessDateTime, orderid, productid, devicesid, status ) VALUES ( '$ipAddress', '$accessDateTime', '$orderid', '$productid', '$devicesid', 'request' )";
...
#-------------------------------------------------------------------------------+
$query = "SELECT id FROM pnmx_license_requests WHERE orderid = '$orderid' AND accessDateTime = '$accessDateTime'";
if ( $debug ) printf( "query=$query<br>" );
...
#-------------------------------------------------------------------------------+
# the next gate is deliberately based on download_requests, not license_requests 
...
{	$row = mysql_fetch_assoc( $result );
	if ( $row['devicesid'] != $devicesid )
		die;					# downloaded by a different device
}
# duplicate 'complete' download gate passed
#-------------------------------------------------------------------------------+
#						capture order (user) information						|
#-------------------------------------------------------------------------------+
$query = "SELECT firstname, lastname, email FROM xcart_orders WHERE orderid = '$orderid'";
if ( $debug ) printf( "query=$query<br>" );
...
$email = $row['email'];
$clrToken = sprintf( "?fn=%s?ln=%s?em=%s?dv=%s?uu=%s?hu=%s", $firstName, $lastName, $email, $deviceid, $uuid, $huid );
$lenToken = strlen( $clrToken );
if ( $debug ) printf( "clrToken=$clrToken<br>" );
#-------------------------------------------------------------------------------+
#						log 'download' in license_requests						|
#-------------------------------------------------------------------------------+
$query = "UPDATE pnmx_license_requests SET status = 'download' WHERE id = '$requestid'";
if ( $debug ) printf( "query=$query<br>" );
$result = mysql_query( $query, $link ) or die( "Requests U1 failed: " . mysql_error() );
#-------------------------------------------------------------------------------+
#						encrypt device and user information						|
#-------------------------------------------------------------------------------+
$clrFile = $ranName . ".l";
$fp = fopen( $clrFile, "w" );
if ( $fp == NULL )
...
$bytesWritten = fwrite( $fp, $clrToken, $lenToken );
if ( $bytesWritten != $lenToken )
{	if ( $debug )
		printf( "wrote %d bytes to %s, should have been=%d<br>", $bytesWritten, $clrFile, $lenToken );
	die();
}
fclose( $fp );

$crpFile = $ranName . ".m";
$lastline = system( "../cgi-bin/rj e ./cfg/$progName.cfg $clrFile $crpFile" );
if ( $lastLine != "" )
{	if ( $debug )
...
passthru( "cat $crpFile" );

# cleanup temp files
$lastline = system( "rm $clrFile $crpFile" );
if ( $lastLine != ""  &&  $debug )
{	if ( $debug )
		printf( "system(rm) returned...<br>%s<br>", $lastLine );
	die();
...
#-------------------------------------------------------------------------------+
#						log 'complete' in license_requests						|
#-------------------------------------------------------------------------------+
$query = "UPDATE pnmx_license_requests SET status = 'complete' WHERE id = '$requestid'";
