...
{	# returns the filename containing the invoice header unless something failed, then false
	...
	$debugFileName = "make_invoice_header_debug.txt";
	
	$tgtFile = $installKeyDir . $orderid . ".dat";
	
	if ( $debug )
	{	$debugFileName = "/home/pnmxcom/public_html/Install_Process/make_invoice_header_debug.txt";
		$lastline = system( "pwd > $debugFileName" );			# when changing order status from the admin page, pwd = /Store/admin
		$fp_dbg = fopen( $debugFileName, "w" );
		if ( $fp_dbg ) fprintf( $fp_dbg, "tgtFile=%s\r\n", $tgtFile );
	}

	$b1 = file_exists( $tgtFile );
	if ( $fp_dbg ) fprintf( $fp_dbg, "fileExists -> %d\r\n", $b1 );
	if ( $b1 )
	{	if ( $fp_dbg ) fclose( $fp_dbg );
		return $tgtFile;			# do not recrypt what we already have
	}
	...
	$tmpFile = "/home/pnmxcom/public_html/Install_Process/tmp/" . MakeRanName() . ".z";
	$fp = fopen( $tmpFile, "w" );
	if ( $fp == NULL )
	{   if ( $fp_dbg )
			fprintf( $fp_dbg, "Couldn't open(w) %s<br>", $tmpFile );
		die();
	}
	...
	{	if ( $fp_dbg )
			fprintf( $fp_dbg, "wrote %d bytes to %s, should have been=%d<br>", $bytesWritten, $tmpFile, $slen );
		die();
	}
	fclose( $fp );
	
	$lastline = system( "/home/pnmxcom/public_html/cgi-bin/rj e /home/pnmxcom/public_html/Install_Process/cfg/token.cfg $tmpFile $tgtFile" );  
	if ( $lastLine != "" )
	{	if ( $fp_dbg )
			fprintf( $fp_dbg, "system(rc) returned...<br>%s<br>", $lastLine );
		die();
	}
	...
	if ( $lastLine != "" )
	{	if ( $fp_dbg )
			fprintf( $fp_dbg, "system(rm) returned...<br>%s<br>", $lastLine );
		die();
	}
