...
#include "NumEdit.h"
#include "atlconv.h"				// W2CA(), USES_CONVERSION
#include "math.h"
...
//--------------------------------------------------------------------
/*			// never gets called
BOOL	CNillaDialog::OnIconNotify( void )
{		// should return value = ! ShowWindow(), or always be nonzero?
	ShowWindow( SW_SHOWNORMAL );	// returns 0 if the window was hidden
...
//	double dd = wcstod( buf, &buf+len );		// wipes out the stack!
	return	(float)atof( buf2 );
}				// getEditBoxFloat()
//----------------------------------------------------------------------------------------
short  CNillaDialog::getComboBoxFloat( int dialogItemID, float& ff )
{		// read the contents of the dialogItemID CEdit control
	CSeekComboBox* scb = (CSeekComboBox*)GetDlgItem( dialogItemID );
	return	scb->GetFloat( ff );
}				// getComboBoxFloat()
//----------------------------------------------------------------------------------------
/*
