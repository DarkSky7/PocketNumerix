...
#include "DBFile.h"
#include "BOS.h"
#include "utils.h"									// PackOleDate, UnpackOleDate
...
{
#ifdef _DEBUG
	TRACE( _T("DataManager::CDataManager(%s)\n"), subDir );
#endif
/*
	CString	appName;
...
#ifdef _DEBUG
	TRACE( _T("DataManager::CDataManager: theApp.GetDBdir --> %s\n"), dbDir );
#endif
...
CMapStringToPtr*	CDataManager::GetSymbolTable( void )
{
	...
	unsigned long	flen = (unsigned long)symFile->GetLength();		// see GetLength for TRY/CATCH example
	symFile->SeekToBegin();
...
{		// return value is the offset of the Asset's symbol (in the symbols file).
	...
	if ( offset != defFile->Seek( offset, CFile::begin ) )
	{
#ifdef _DEBUG
...
{		// position for reading a BOS definition
	...
	if ( offset != (DWORD)defFile->Seek( offset, CFile::begin ) )
	{
#ifdef _DEBUG
...
//////////////////////////////////////////////////////////////////////////////
//----------------------------------------------------------------------------
long	CDataManager::GetIndexedDef_ID( long zbi )
{		// returns the def_ID located at 4*zbi in the posIndxFile
	...
	{
#ifdef _DEBUG
		TRACE( _T("DataManager::GetIndexedDef_ID: Error(-2) seeking to %d.\n"), offset );
#endif
		return	-2;
	}
		// read the def_ID
	...
	{
#ifdef _DEBUG
		TRACE( _T("DataManager::GetIndexedDef_ID: Error(-3) reading a def_ID from the posIndx file.\n") );
#endif
		return	-3;
	}
	return	def_ID;
...
{	
	...
	return	(long)((unsigned long)posIndxFile->GetLength() / sizeof(long));
}			// NumIndexedIssues()
//----------------------------------------------------------------------------
...
{		// get the index file
	...
	int		res = (unsigned long)posIndxFile->Seek( 0 - sizeof(lastDef_ID), CFile::end );
	...
	{		// Yes.  We have to write lastDef_ID into the bos's slot (bos->posIndx_ID)
		...
		if ( bos->posIndx_ID != posIndxFile->Seek( bos->posIndx_ID, CFile::begin ) )
		{
#ifdef _DEBUG
		...
		CBOS*	lastBos = ReadBOS( lastDef_ID );
		lastBos->posIndx_ID = bos->posIndx_ID;
...
{		// Assumes posFile is already correctly positioned for the read
	...
	wchar_t		pckdInstStat;		// {Bond, Option, Stock} & {Closed, Deleted, Open}
	long		def_ID;				// offset into <bond/option/stock>Defs.pnh
	long		pckdPurchDate;
	double		nUnits;
	double		totalCost;
...
{		// Assumes posFile is already correctly positioned for the write
	...
	{		// no longer storing the status byte for Open Positions
		...
		posFile->Write( &pckdInstStat,	1					  );	// { Bond, Option, Stock } & { Closed, Deleted, Open }
...
CPosition*		CDataManager::ReadOpenPosition( long offset )
{
	...
	if ( offset != posFile->Seek( offset, CFile::begin ) )
	{
#ifdef _DEBUG
...
{		// write a closed CPosition back to the file system (object store)
	...
	long	positionOffset = (unsigned long)posFile->Seek( 0, CFile::end );
...
{
	ASSERT( strFile != NULL );
	if ( offset != strFile->Seek( offset, CFile::begin ) )
	{
#ifdef _DEBUG
	...
	fsLen &= 0xFF;
	if ( offset != strFile->Seek( -1, CFile::current ) )
	{
#ifdef _DEBUG
...
wchar_t*		CDataManager::ReadPString( CDBFile* fp, long offset )
{
	...
	if ( offset != fp->Seek( offset, CFile::begin ) )
	{
#ifdef _DEBUG
...
{		// strFile is assumed to be open (but not positioned) for read/write
	...
	cs.TrimRight();
	short	bytesToWrite;
	short	zeroFill = 0;					// number of wchar_t's to NULL out after writing cs
	...
	{		// we hope to replace an existing PString stored at offset
		...
		unsigned long	flen = (unsigned long)strFile->GetLength();
		if ( zeroFill > 0  ||  flen == ((unsigned long)offset + origBytes) )
