...
BOOL	CPositionsDialog::OnInitDialog() 
{
	...
	RegisterHotKeys( IDC_Symbol, IDC_PositionsList );	// screens for WM5 internally
	...
	if ( theApp.GetSymbolTableSize(Bond) > 0 )
	{		// a non-empty symbol table is present
		LoadSymbols();
	...
	COleDateTime	minDate( 100, 1, 1, 0, 0, 0);
...
{	
	...
	c_Symbol.LoadMap( theApp.bndMgr->GetSymbolTable() );
...
//----------------------------------------------------------------------------------------
void	CPositionsDialog::LoadTheAsset( void )
{		// Reset accumulators
	aggUnits = 0.0;
	aggCost = 0.0;
	aggIncome = 0.0;
...
{		// sets theAsset as a side effect
	...
	theAsset = theApp.bndMgr->GetBond( m_Symbol );
	if ( theAsset == NULL )
	{	if ( !create )
			return	NULL;

			// didn't find m_Symbol in the bonds map, but create is enabled
			// so this will be a new entry
		CBond*	aBond = new CBond( m_Symbol );
#ifdef _DEBUG
		ASSERT_VALID( aBond );
#endif
...
*/
		...
		// if necessary, create a Bond, Option, or Stock, so we can attach the position to it
...
{		// the following buffer operation on m_Symbol would seem equivalent to
		// m_Symbol.IsEmpty(), but that returns true after a switch from Bond
		// or Stock to Option, even when m_Symbol is non-empty as evidenced by
...
//----------------------------------------------------------------------------------------
void	CPositionsDialog::OnKillFocusUnits( void )
{
	SHSipPreference( GetSafeHwnd(), SIP_DOWN );
...
{		// return a CString representation of the data in row rr, subitem si
	...
	unsigned short	lastCol = 8;			// zero based
...
#ifdef _DEBUG
	ASSERT ( pItem != NULL  &&  theAsset != NULL );
#endif
		// get the CString for the requested subitem (si)
	wchar_t	wbuf[20];				// where we construct the return string
	while ( 1 )
	{
		...
		{	CPosition*	pos = pItem->thePosition;
			if ( msi == 1 )
			{		// Its the # of Units (shares/options/bonds) in the Position or total row
				swprintf( wbuf, _T("%g"), pos ? pos->nUnits : aggUnits );
			}
			else														// msi == 2
			{		// Its the cost of the Position or total row
				swprintf( wbuf, _T("%.2f"), pos ? pos->totalCost : aggCost );
			}
			break;
		}
		if ( msi == 3 )
		{		// Its the market value of the Position
			swprintf( wbuf, _T("%.2f"), pItem->mktValue );
			break;
		}
			// capital gain is 4th (zero based)
		if ( msi == 4 )
		{
			swprintf( wbuf, _T("%.2f"), pItem->capGain );
			break;
		}

			// income is 5th for Stocks/Bonds, exerVal is 5th (zero based) for Options
		if (  msi == 5 )
		{		// dividend or coupon income (zero based 5th for Stocks & Bonds)
				//			  exercise value (zero based 6th for Options)
			swprintf( wbuf, _T("%.2f"), pItem->income );
			break;
		}
		...
		{		// NetGain
			...
			swprintf( wbuf, _T("%.2f"), netGain );
			break;
		}

			// the only thing left is pctYield
			// pctYield is 5th for Options, but 7th for Stocks & Bonds
		double	plusYld = fabs(pItem->pctYield);
		swprintf( wbuf, ((plusYld <   10.0) ? _T("%.3f")
					  : ((plusYld <  100.0) ? _T("%.2f")
...
{		// this is a column sort event
	...
	short sortKey = MapSubitem( pNMLV->iSubItem ) + 1;			// why + 1 ???
/*
...
//----------------------------------------------------------------------------------------
// delete the given item from the ListCtrl and from the Stock/Bond/Option's Positions list
...
//	double	yrs = timeSpan.GetTotalDays() / 365.0;			// XXX need something more accurate?
	...
	double	exerVal = 0.0;
	double	income = ((CBond*)theAsset)->PresentValueCoupons( pos->purchaseDate, today, 0.0 );
	income *= pos->nUnits;											// return value
...
{		// ensure that the next sort on the first column will be ascending
	...
	if ( ! GetPosListColVisibility( &areVisible ) )
	{
		...
		TRACE( _T("PositionsDialog::InitListCtrl: no column visibility preferences found.\n") );
															  ...
															  // msi =  Bond
	optns[op++] = (areVisible & PurchaseDate)			!= 0;		//    0
	optns[op++] = (areVisible & NumUnits)				!= 0;		//	  1
	optns[op++] = (areVisible & InitialCost)			!= 0;		//	  2
	optns[op++] = (areVisible & MarketValue)			!= 0;		//	  3
	optns[op++] = (areVisible & CapitalGain)			!= 0;		//	  4

		// options don't have income, but bonds/stocks do
	optns[op++] = (areVisible & AggregateIncome)		!= 0;		//	  5
	optns[op++] = (areVisible & NetGain)				!= 0;		//	  6

		// annualizedNetGain is pctYield...
	optns[op++] = (areVisible & AnnualizedNetGain)		!= 0;		//	  7
	optns[op++] = (areVisible & Note)					!= 0;		//	  8
...
{		// delete any existing columns
	...
	wchar_t*		colHdrs[] = {
		...
		_T("Income"), _T("NetGain"), _T("% Yield"),  _T(" Note")					  };
...
//----------------------------------------------------------------------------------------
void	CPositionsDialog::OnKillFocusNote()
{	CString note;
	SHSipPreference( GetSafeHwnd(), SIP_DOWN );
...
{		// respond to Ctrl-K (kill) keyboard command
	...
	long	def_ID = bm->AssetExists( m_Symbol );
	if ( def_ID < 0 )
		return;

	CBOS*	bos = bm->ReadBOS( def_ID );
	if ( bos == NULL )
		return;
	...
	{	wchar_t	buf[400];
		swprintf( buf, _T("Deleting '%s' will irrevocably delete its %d positions. Okay to proceed?"),
					m_Symbol, nItems );
		CString	caption = _T("Delete bond/positions?");
...
//----------------------------------------------------------------------------------------
/*
void CPositionsDialog::OnSelEndOkSymbol()
{
	OnKillFocusSymbol();
}			// OnSelEndOkSymbol()
*/
...
{	BOSPositionListItemInfo*	lps1 = (BOSPositionListItemInfo*)lp1;
	...
	{		// What column is deciding our destiny?
		...
		{		// Its the capital gain for this Position
			...
			// Bond/Option/Stock's pctYield & note are both handled above
