...
{	WIN32_FIND_DATA		findFileData;
	...
	{	CString	dbDir = bm->GetDBdir();
		...
		hSearch = FindFirstFile( openPosFileName, &findFileData );
	...
	c_YearList.SetCurSel( idx == LB_ERR  ?  cnt - 1  :  idx );	// if no Open, use the last (most recent) entry
	OnSelChangeYearList();
...
{
	m_activeComboBox = IDC_YearList;						// HotKeys
...
{
	int	cnt = c_YearList.GetCount();
	if ( cnt < 1  ||  item < 0  ||  cnt <= item  ||  item == m_itemSelected )
		return;
	m_itemSelected = item;
	CString yrSt;
	COleDateTime toDate;							// closedPositions get toDate assigned later
	wchar_t	buf[1024];
		// hysteresis handled in OnSelChangeYearList(), so we can just load 'em up
		// folderNames coincide with the order we loaded the dataManagers in { Bond, Option, Stock }
	const CString folderName = _T("Bond");
	...
	{	int		nPos = 0;					// number of positions in the folder
		...
		PositionIterationContext*	ctx = bm->GetPositionIterationContext( yr );
		while ( ctx != NULL )
		{	CPosition* pos = bm->GetNextPosition( ctx );
			if ( pos == NULL )
				break;
...
//				TRACE( _T("2: %s\n"), buf );
			...
			{		// Stocks and Bonds
				...
				// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
				// calculate the netGain for everything, but just display it for bonds
			...
			{	swprintf( buf, _T("Net Gain: %.2f\r\n"), netGain );
				...
				// append the pctYield
			long	spDays;
			double	yrs = calcYears( pos->purchaseDate, toDate, ActualActual, spDays );
	...
	fp->Close();
	delete	fp;								// XXX should we delete the file on disk too?
...
{		// FALSE indicates Save As... functionality
	...
	try
#endif
	{	fp.Write( &transText, (unsigned long)transText.GetLength() );
	}
#if ( SEH_EXCEPTIONS )
...
{		// GetCurSel() returns the value AFTER the change takes affect on PPC02 & PPC03, but
		// BEFORE the change takes effect on WM5 (because we're setting lastChar manually)
	int cnt = c_YearList.GetCount();
	if ( cnt < 1 )
		return;
...
#endif
	if ( sel < 0 ) sel = 0;			// this should never happen
	itemSelected = sel;
	LoadTranscript( itemSelected );
}			// OnSelChangeYearList()
//---------------------------------------------------------------------------
/*
void	CPositionTranscriptDialog::OnEditUpdateYearList( void )
{	CString sym;
	c_YearList.GetWindowText( sym );
	sym.TrimLeft();
	sym.TrimRight();
	int	res = c_YearList.FindStringExact( -1, sym );

	c_SaveTranscript.EnableWindow( res >= 0 );
	if ( res >= 0 )
		itemSelected = res;
}			// OnEditUpdateYearList()
//---------------------------------------------------------------------------
void	CPositionTranscriptDialog::OnSelChangeYearList( void )
{
	int res = c_YearList.GetCurSel();
